		...
		bucketPolicy := `
{
    ...
    "Statement": [
		...
		t.Run("create obc with bucketPolicy", func(t *testing.T) {
			...
			assert.True(t, obc.Spec.AdditionalConfig["bucketPolicy"] == bucketPolicy)
		...
		t.Run("policy was applied verbatim to bucket", func(t *testing.T) {
			...
			assert.Equal(t, bucketPolicy, *policyResp.Policy)
		})
		...
		t.Run("policy was removed from bucket", func(t *testing.T) {
			...
			if aerr, ok := err.(awserr.Error); ok {
				assert.Equal(t, "NoSuchBucketPolicy", aerr.Code())
			} else {
				assert.Fail(t, "expected NoSuchBucketPolicy error")
			}
		})

...
#
# XXX Should the CephCluster customization be elimated from this function?
...
# deploying a smorgasbord of resources used by multiple canary tests.  All
# canary tests using this function should be refactored to only deploy only the
# minimal set of resources needed.
function deploy_cluster_full_of_cruft_please_stop_using_this() {
  deploy_cluster "$@"
